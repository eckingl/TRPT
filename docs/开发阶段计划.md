...
2. **选择专题**：
   - 土壤普查：属性图、类型图、适宜性评价
   - 耕地质量：等级评价
3. **选择地区**：支持新建（省市县三级）和选择已有地区
4. **选择操作**：上传数据、配置参数、预览数据、生成报告

#### 省市县选择
- 支持省份：河南省、安徽省、江苏省、浙江省
- 市级：每省 10-12 个主要城市
- 县级：手动输入
- 存储格式：完整名称（如"河南省郑州市中牟县"）

#### 数据库结构
- `regions` 表：存储地区信息（id, name, category, topic, item, province, city, county, created_at, updated_at）
- `project_data` 表：存储项目数据
- `project_config` 表：存储项目配置
...
1. [x] 后端可启动，访问 `http://localhost:8000/api/health` 返回成功
2. [x] 前端可启动，访问 `http://localhost:5173` 显示界面
3. [x] 前端可调用后端 API
4. [x] 代码通过 linter 检查
