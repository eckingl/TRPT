...
#### 地区管理重构
- [x] 地区按大类(category)共享，不再绑定到具体专题
- [x] 数据库唯一约束改为 `UNIQUE(name, category)`
- [x] `topic` 和 `item` 字段改为可选

#### 专题配置系统
- [x] `backend/app/core/topic_config.py` - 专题配置核心模块
  - `TopicConfigManager` 管理配置文件
  - 配置文件路径: `data/configs/{topic}/{region_id}.json`
  - 支持属性图和数据报告两种配置类型

- [x] `backend/app/api/topic_config.py` - 专题配置 API
  - `GET /api/topic-config/{topic}/{region_id}` - 获取配置
  - `POST /api/topic-config/{topic}/{region_id}` - 创建或获取配置
  - `PUT /api/topic-config/{topic}/{region_id}/grading-standard` - 更新分级标准
  - `PUT /api/topic-config/{topic}/{region_id}/attribute-map/*` - 属性图配置
  - `PUT /api/topic-config/{topic}/{region_id}/data-report/*` - 数据报告配置

#### 分级标准系统
- [x] `backend/app/core/grading_standards.py` - 分级标准注册系统
  - 独立的分级标准定义
  - 默认标准: `jiangsu` (江苏分级)
  - 支持扩展其他分级标准

- [x] `backend/app/api/grading.py` - 分级标准 API
  - `GET /api/grading/standards` - 获取所有分级标准
  - `GET /api/grading/standards/current` - 获取当前分级标准
  - `GET /api/grading/attributes` - 获取属性配置

#### 数据报告专题
- [x] 新增 `data_report` 作为独立专题
- [x] `DataReportConfig` 配置模型
  - `DataReportDataConfig` - 数据处理配置
  - `DataReportStatsConfig` - 统计配置
  - `DataReportOutputConfig` - 输出配置

#### 前端更新
- [x] `Home.vue` - 主页重构
  - 地区选择在选择大类后显示（不需要先选专题）
  - 每个专题有独立的操作列表 (`topicActions`)
  - `currentActions` 计算属性动态显示操作

- [x] `Config.vue` - 配置页面
  - 支持数据报告专题配置
  - 数据处理、统计、输出配置面板

- [x] `api/index.js` - 新增 API 函数
  - 数据报告配置相关 API
...
| 专题 ID | 名称 | 可用操作 |
|---------|------|----------|
| `attribute_map` | 属性图 | 数据处理、上传数据、配置参数、生成报告 |
| `type_map` | 类型图 | 上传数据、配置参数、生成报告 |
| `suitability` | 适宜性评价 | 上传数据、配置参数、生成报告 |
| `data_report` | 数据报告 | 生成数据报告、上传数据、配置参数 |
| `grade_eval` | 等级评价 | 上传数据、配置参数、生成报告 |
...
- [ ] 类型图专题 (`type_map`)
- [ ] 适宜性评价专题 (`suitability`)
- [ ] 数据报告专题 (`data_report`) - 配置已完成，功能待实现
- [ ] 等级评价专题 (`grade_eval`)

### 每个专题需要

1. `topics/{topic_name}/config.py` - 配置定义
2. `topics/{topic_name}/process.py` - 数据处理
3. `topics/{topic_name}/generate.py` - 报告生成
4. 对应的 Word 模板
5. 单元测试
...
│   │   │   ├── grading.py           # 分级标准 API ⭐ P6新增
│   │   │   └── topic_config.py      # 专题配置 API ⭐ P6新增
│   │   ├── core/
│   │   │   ├── __init__.py
│   │   │   ├── database.py          # SQLite 数据库
│   │   │   ├── grading_standards.py # 分级标准定义 ⭐ P6新增
│   │   │   ├── topic_config.py      # 专题配置管理 ⭐ P6新增
...
│   │   ├── main.js
│   │   ├── views/
│   │   │   ├── Home.vue             # 主页（专题操作列表）⭐ P6更新
│   │   │   ├── Upload.vue
│   │   │   ├── Config.vue           # 配置页 ⭐ P6更新
│   │   │   ├── Report.vue
│   │   │   ├── AttributeProcess.vue
│   │   │   └── DataReport.vue
│   │   ├── api/
│   │   │   └── index.js             # API 函数 ⭐ P6更新
│   │   └── stores/
│   │       └── project.js
│   ├── package.json
│   └── vite.config.js
...
| 日期 | 修改内容 |
|------|----------|
| 2025-12-09 | P6 阶段完成：专题-地区配置架构重构，地区按大类共享，每个专题独立操作列表 |
